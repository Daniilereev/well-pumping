...
function calculateone() {
  ...
  trans = document.getElementsByClassName('in')[0].value;
  pezo = document.getElementsByClassName('in')[1].value;
  debit = document.getElementsByClassName('in')[2].value;
  radius = document.getElementsByClassName('in')[3].value;
  time = document.getElementsByClassName('in')[4].value;
  console.log(trans, pezo, debit, radius, time);
...
buttontwo.onclick = function () {
    ...
    for (let i = 0; i<quan.value*2; i=i+2) {
          if (data[i].value!='' || data[i+1].value !='') {
            ...
            onewell = q / 4 / 3.14 / trans * Math.log(2.25 * pezo * time / r / r);
            if (onewell > 0) {
